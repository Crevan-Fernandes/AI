...
# Print the full truth table nicely
...
# Print filtered truth table for KB and an entailment column (minimal columns)
def print_filtered_truth_table_minimal(truth_table, entailment_index, entailment_name):
    header = ["KB", entailment_name]
    print(f"\nTruth table rows where KB is True showing only KB and {entailment_name}:")
    print(" | ".join(header))
    print("-" * 20)
    ...
    for row in truth_table:
        kb_true = row[6]
        if kb_true:
            filtered_row = [row[6], row[entailment_index]]
            formatted_row = [("T" if val else "F") for val in filtered_row]
            print(" | ".join(formatted_row))
...
if __name__ == "__main__":
    ...
    print_filtered_truth_table_minimal(truth_table, 7, "R")
    print(f"Does KB entail R? {check_entailment(truth_table, 7)}")

    print_filtered_truth_table_minimal(truth_table, 8, "R → P")
    print(f"Does KB entail R → P? {check_entailment(truth_table, 8)}")

    print_filtered_truth_table_minimal(truth_table, 9, "Q → R")
